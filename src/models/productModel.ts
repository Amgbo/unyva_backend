...
export const searchProducts = async (searchParams: {
  ...
  try {
    let query = `
      ...
      FROM products p
      LEFT JOIN students s ON p.student_id = s.student_id
      LEFT JOIN university_halls sh ON s.hall_of_residence = sh.full_name
      LEFT JOIN product_images pi ON p.id = pi.product_id
    ...
    // Single hall filter (filter by seller's hall)
    if (searchParams.hall_id) {
      query += ` AND sh.id = $${values.length + 1}`;
      values.push(searchParams.hall_id);
    }

    // Multiple halls filter (filter by seller's halls)
    if (searchParams.halls && searchParams.halls.length > 0) {
      query += ` AND sh.id = ANY($${values.length + 1}::int[])`;
    ...
    query += ` GROUP BY p.id, sh.full_name, s.first_name, s.last_name`;
...
export const getSearchFilters = async () => {
  try {
    ...
    const [categoriesResult, conditionsResult, hallsResult, priceRangesResult] = await Promise.all([
      ...
      // Halls (show all available university halls)
      pool.query(`
        SELECT
          array_agg(json_build_object('id', h.id, 'name', h.full_name)) as values,
          count(*) as count
        FROM university_halls h
